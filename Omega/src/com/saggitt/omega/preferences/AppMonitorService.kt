...
class AppMonitorService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        val exit = Utilities.getOmegaPrefs(baseContext)::exitHidden.get()
    ...
    private fun startMonitoring() {
        val customAppFilter = CustomAppFilter(baseContext)
        val omegaAppFilter = OmegaAppFilter(baseContext)
        val config = Config(baseContext)
        ...
        handler.post(object : Runnable {
            override fun run() {
                if (isRunning) {
                    ...
                    val appsList = config.getAppsList(null)
                    val size = appsList.count {
                        if (it.componentName.packageName == foregroundApp) {
                          customAppFilter.isHiddenPackage(baseContext, it.componentName.packageName )// 自定义不展示展示
                    ...
                    handler.postDelayed(this, 2000)  // 每秒执行
                }
            }
        })
